...
def plan_routes_with_precomputed_paths(G, hub_node, delivery_points, truck_capacity, truck_type, shortest_paths, time_limit):
    '''
    Plan delivery routes for a truck type given the graph, hub, delivery points, and shortest paths.
    Returns the total time, trip count, and deliveries completed within the time limit.
    '''
    ...
    return total_time, trip_count, deliveries_completed


# Run a single simulation for Truck A or Truck B
def run_single_simulation(args):
    G, hub_node, delivery_points, truck_capacity, truck_type, shortest_paths, time_limit = args
    return plan_routes_with_precomputed_paths(G, hub_node, delivery_points, truck_capacity, truck_type, shortest_paths, time_limit)
...
def run_monte_carlo_simulation(place_name, initial_deliveries, num_simulations, increment, time_limit):
    ...
    for simulation in range(1, num_simulations + 1):
        ...
        args_a = (G_scc, hub_node, delivery_points, 200, "Truck A (Large)", shortest_paths_truck_a, time_limit)
        args_b1 = (G_scc, hub_node, delivery_points[:len(delivery_points) // 2], 100, "Truck B1 (Small)", shortest_paths_truck_b, time_limit)
        args_b2 = (G_scc, hub_node, delivery_points[len(delivery_points) // 2:], 100, "Truck B2 (Small)", shortest_paths_truck_b, time_limit)

        with ProcessPoolExecutor(max_workers=3) as executor:
            results = executor.map(run_single_simulation, [args_a, args_b1, args_b2])

        (time_a, trips_a, deliveries_a), (time_b1, trips_b1, deliveries_b1), (time_b2, trips_b2, deliveries_b2) = results

        time_b = max(time_b1, time_b2)
        trips_b = trips_b1 + trips_b2
        deliveries_b = deliveries_b1 + deliveries_b2

        # Calculate percentages based on deliveries completed within the time limit
        truck_a_percentage = (deliveries_a / num_deliveries) * 100 if deliveries_a < num_deliveries else 100
        truck_b_percentage = (deliveries_b / num_deliveries) * 100 if deliveries_b < num_deliveries else 100
